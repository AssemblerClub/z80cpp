...
void 
Z80::exe_POP_rp(uint8_t& rhi, uint8_t& rlo) {
   m_ops.addM45Read( m_reg.SP, rlo, TZ80Op(&Z80::inc, m_reg.SP) );
   m_ops.addM45Read( m_reg.SP, rhi, TZ80Op(&Z80::inc, m_reg.SP) );
}
...
Z80::decode() {
   ...
   switch( m_data ) {
      ...
      case 0xF1: exe_POP_rp    ( rm.A, rm.F);      break;
